...
#[cfg(test)]
mod tests {
	use co_dilithium_crypto::{DilithiumPublic, DilithiumSignature};
	...
	fn test_dilithium_extrinsic() {
		...
		let keypair =
			co_dilithium_crypto::generate(Some(&entropy)).expect("Failed to generate keypair");
	...
	fn test_dilithium_extrinsic_fail_signature() {
		...
		let keypair2 =
			co_dilithium_crypto::generate(Some(&entropy2)).expect("Failed to generate keypair");
