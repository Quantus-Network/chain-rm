...
mod tests {
	...
	fn full_recovery_cycle_works() {
		TestCommons::new_test_ext().execute_with(|| {
			...
			println!("Initial recovery account balance: {}", initial_recovery_balance);
			...
			let close_recovery_call =
				Box::new(RuntimeCall::Recovery(pallet_recovery::Call::close_recovery {
			...
			// 5. As the recovery account, execute a `transfer_all` call on behalf of the lost
			//    account.
			let transfer_all_call =
				Box::new(RuntimeCall::Balances(pallet_balances::Call::transfer_all {
					...
					keep_alive: false, /* Drains the account, but leaves existential deposit due
					                    * to fees. */
				}));
			...
			// Note: We use the values from before the final transfer because both accounts paid
			// some fees
