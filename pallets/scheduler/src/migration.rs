...
pub mod v3 {
    ...
    impl<T: Config> OnRuntimeUpgrade for MigrateToV4<T> {
        ...
        fn pre_upgrade() -> Result<Vec<u8>, TryRuntimeError> {
                ...
                "Can only upgrade from version 3"
            );
...
pub mod v4 {
    ...
    impl<T: Config> OnRuntimeUpgrade for CleanupAgendas<T> {
        ...
        fn pre_upgrade() -> Result<Vec<u8>, TryRuntimeError> {
            ...
            let non_empty_agendas = Agenda::<T>::iter_values()
                .filter(|a| a.iter().any(|s| s.is_some()))
                .count();
            log::info!(
                target: TARGET,
        ...
        fn post_upgrade(state: Vec<u8>) -> Result<(), TryRuntimeError> {
            ...
            match old_agendas.checked_sub(new_agendas) {
                ...
                "Expected to keep all non-empty agendas"
            );
...
mod test {
    ...
    fn migration_v3_to_v4_works() {
        new_test_ext().execute_with(|| {
            ...
            let large_call = RuntimeCall::System(frame_system::Call::remark {
                remark: vec![0; 1024],
            });
            // Call that can be inlined.
            let small_call = RuntimeCall::System(frame_system::Call::remark {
                remark: vec![0; 10],
            });
            // Call that is already hashed and can will be converted to `Legacy`.
            let hashed_call = RuntimeCall::System(frame_system::Call::remark {
                remark: vec![0; 2048],
            });
    ...
    fn cleanup_agendas_works() {
        ...
        new_test_ext().execute_with(|| {
            ...
            let test_data: Vec<(
                ...
                (
                    bounded_vec![None, some.clone()],
                    Some(bounded_vec![None, some.clone()]),
                ),
                (
                    bounded_vec![None, some.clone(), None],
                    Some(bounded_vec![None, some.clone()]),
                ),
                (
                    bounded_vec![some.clone(), None, None],
                    Some(bounded_vec![some.clone()]),
                ),
